...
class Parse:
    ...
    def getText(self, text, max_length, top_p, temperature):
        ...
        if result == None:
            exit()
        return result
    def switchAudio(self, text, sc, ns, nsw, ls):
        global audio_path
        audio_path = self.store_path + '/' + str(self.cnt) + '.wav'
        self.cnt += 1
        self.gs.generateSound(text, audio_path, sc, ns, nsw, ls)
        logging.warning('text change to audio successfully!')
        return text, audio_path
    def PipeChat(self, text, max_length, top_p, temperature, sc, ns, nsw, ls):
        return self.switchAudio(self.getText(text, max_length, top_p, temperature), sc, ns, nsw, ls)
